...
full_AΣb(σs, two_λs) = sum(amplitude(ch, σs, two_λs) for ch in [dc_Σb1, dc_Σb3])
...
full_AΣb_x(σs, two_λs) = sum(amplitude(ch, σs, two_λs) for ch in [dc_Σb_x1, dc_Σb_x3])

dc_f0 = DecayChainLS(2, σ -> 1.0; tbs=tbs, two_s=0, parity='+')
full_Af0(σs, two_λs) = amplitude(dc_f0, σs, two_λs)
...
###
one_AΣb(σs, two_λs) = amplitude(dc_Σb1, σs, two_λs)
one_AΣb_x(σs, two_λs) = amplitude(dc_Σb_x1, σs, two_λs)
