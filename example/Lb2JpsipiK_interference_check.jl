...
#
Ini(σs,cs) = sum(abs2(sum(c*amplitude(σs,two_λs,dc) for (c, dc) in zip(cs, Λs ))) for two_λs in itr(tbs.two_js)) +
             sum(abs2(sum(c*amplitude(σs,two_λs,dc) for (c, dc) in zip(cs, Pcs))) for two_λs in itr(tbs.two_js))
#
